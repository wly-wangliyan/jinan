...
import { PublicSecurityHttpService, VehicleAccessEntity } from '../../public-security-http.service';
...
export class RightTwoComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private newDataList: Array<VehicleAccessEntity> = [];
  ...
  public requestData(): void {
    this.dataSubscription && this.dataSubscription.unsubscribe();
    this.dataSubscription = this.securityHttpService.requestVehicleAccessList().subscribe(results => {
   ...
   */
  private convertRollData(lastItem: RollDataItem, newDataList: Array<VehicleAccessEntity>): Array<RollDataItem> {
    const rollDataList = [];
