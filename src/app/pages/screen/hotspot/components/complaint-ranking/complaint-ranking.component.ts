...
import {ComplaintRankingEntity, HotspotService} from '../../hotspot.service';
...
@Component({
  selector: 'app-complaint-ranking',
  templateUrl: './complaint-ranking.component.html',
  styleUrls: ['./complaint-ranking.component.less'],
  providers: [ListScrollService]
})
export class ComplaintRankingComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private newDataList: Array<ComplaintRankingEntity> = [];
  ...
  ngOnInit(): void {
    this.dataSubscription = this.hotPotService.requestComplaintRankingList().subscribe((results: Array<ComplaintRankingEntity>) => {
      // 制作为UI效果定制的数据
   ...
   */
  private convertRollData(lastItem: RollDataItem, newDataList: Array<ComplaintRankingEntity>): Array<RollDataItem> {
    const rollDataList = [];
