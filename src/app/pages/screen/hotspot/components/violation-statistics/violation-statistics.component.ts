...
import {HotspotService, ViolationEntity} from '../../hotspot.service';

const MAX_LIST_COUNT = 5;
const ROW_HEIGHT = 48;

@Component({
  selector: 'app-violation-statistics',
  templateUrl: './violation-statistics.component.html',
  styleUrls: ['./violation-statistics.component.less'],
  providers: [ListScrollService]
})
export class ViolationStatisticsComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private newDataList: Array<ViolationEntity> = [];
  ...
  ngOnInit(): void {
    this.dataSubscription = this.hotPotService.requestViolationList().subscribe((results: Array<ViolationEntity>) => {
      // 制作为UI效果定制的数据
   ...
   */
  private convertRollData(lastItem: RollDataItem, newDataList: Array<ViolationEntity>): Array<RollDataItem> {
    const rollDataList = [];
